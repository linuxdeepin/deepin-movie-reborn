...
//////////////aicl add 1206
#if 0
        s_vpu_reg_base = (unsigned long)mmap(NULL, s_vpu_reg_size, PROT_READ | PROT_WRITE, MAP_SHARED, vdi->vpu_fd, 0);
        if ((void *)s_vpu_reg_base == MAP_FAILED)
        {
                VLOG(ERR, "[VDI] fail to map vpu registers \n");
                goto ERR_VDI_INIT;
        }
        printf("s_vpu_reg_base: %#lx\n", s_vpu_reg_base);
#endif	
///////////////////////
