    ...
    : QWidget(mw), _engine(mpv), _mw(static_cast<Platform_MainWindow *>(mw))
{
    ...
    m_pClearButton = new Platform_ClearButton();
    ...
    connect(m_pClearButton, &Platform_ClearButton::clearList, _engine, &PlayerEngine::clearPlaylist);
...
Platform_ClearButton::Platform_ClearButton(QWidget *parent)
    :QPushButton (parent)
{
    m_bIsHover = false;
    m_bIsPressed = false;
...
void Platform_ClearButton::paintEvent(QPaintEvent *pe)
{
    QPainter painter(this);
    painter.setPen(Qt::NoPen);
//    QImage img;
...
void Platform_ClearButton::enterEvent(QEvent *event)
{
    m_bIsHover = true;
    update();
...
void Platform_ClearButton::leaveEvent(QEvent *event)
{
    m_bIsHover = false;
    update();
...
void Platform_ClearButton::mousePressEvent(QMouseEvent *event)
{
    m_bIsPressed = true;
    update();
...
void Platform_ClearButton::mouseReleaseEvent(QMouseEvent *event)
{
    m_bIsPressed = false;
    emit clearList();
    update();
