    ...
    : QObject(nullptr)
{
    ...
    [ = ](const QString & key, const QVariant & value) {
        if (key.startsWith("shortcuts."))
            ...
            if (index  == 1) {
                ...
                if (voFamily)
                    voFamily->setData("items", QStringList() << "auto");
            } else {
                if (voFamily) {
                    voFamily->setData("items", QStringList() << "auto" << "gpu" << "vaapi" << "vdpau" << "xv" << "x11");
                }
            }
            ...
            if (vo.contains("vaapi")) {
                ...
                auto decodeFamily = m_pSettings->option("base.decode.Decodemode");
                if (decodeFamily)
                    decodeFamily->setData("items", QStringList() << "auto" << "vaapi" << "vaapi-copy" << "vdpau" << "vdpau-copy" << "nvdec" << "nvdec-copy" << "rkmpp");
            }
            emit baseChanged(key, value);
        }
    ...
    hwdecList << "auto" << "vaapi" << "vaapi-copy" << "vdpau" << "vdpau-copy";
    voList << "auto" << "gpu" << "vaapi" << "vdpau" << "xv" << "x11";
    auto hwdecFamily = m_pSettings->option("base.decode.Decodemode");
    if (hwdecFamily)
        hwdecFamily->setData("items", hwdecList);
    
    int index = m_pSettings->getOption("base.decode.Decodemode").toInt();
    QString decode = hwdecFamily.data()->data("items").toStringList().at(index);
    if (decode.contains("vaapi")) {
        auto voFamily = m_pSettings->option("base.decode.Videoout");
        if (voFamily)
            voFamily->setData("items", QStringList() << "vaapi" << "gpu");
    } else if (decode.contains("vdpau")) {
        auto voFamily = m_pSettings->option("base.decode.Videoout");
        if (voFamily)
            voFamily->setData("items", QStringList() << "vdpau" << "xv" << "x11");
    } else {
        auto voFamily = m_pSettings->option("base.decode.Videoout");
        if (voFamily)
            voFamily->setData("items", QStringList() << "auto" << "gpu" << "vaapi" << "vdpau" << "xv" << "x11");
