...
#include "platform_dmr_settings.h"
...
Platform_Settings *Platform_Settings::m_pTheSettings = nullptr;
Platform_Settings &Platform_Settings::get()
{
    if (!m_pTheSettings) {
        m_pTheSettings = new Platform_Settings;
    }
...
QString Platform_Settings::flag2key(Platform_Settings::Flag f)
{
    switch (f) {
    case Platform_Settings::Flag::ClearWhenQuit:
        return "emptylist";
#ifndef __aarch64__
    case Platform_Settings::Flag::ShowThumbnailMode:
        return "showInthumbnailmode";
#endif
    case Platform_Settings::Flag::ResumeFromLast:
        return "resumelast";
    case Platform_Settings::Flag::AutoSearchSimilar:
        return "addsimilar";
    case Platform_Settings::Flag::PreviewOnMouseover:
        return "mousepreview";
    case Platform_Settings::Flag::MultipleInstance:
        return "multiinstance";
    case Platform_Settings::Flag::PauseOnMinimize:
        return "pauseonmin";
    }
...
bool Platform_Settings::isSet(Flag flag) const
{
    bool bRet = false;
...
QStringList Platform_Settings::commonPlayableProtocols() const
{
...
bool Platform_Settings::iscommonPlayableProtocol(const QString &sScheme) const
{
...
QString Platform_Settings::screenshotLocation()
{
...
QString Platform_Settings::screenshotNameTemplate()
{
...
//cppcheck 单元测试使用
QString Platform_Settings::screenshotNameSeqTemplate()
{
    return tr("%1/Movie%2(%3).jpg").arg(screenshotLocation())
           .arg(QDateTime::currentDateTime().toString("yyyyMMddhhmmss"));
}

void Platform_Settings::onSetCrash()
{
    settings()->setOption(QString("set.start.crash"),false);
    settings()->sync();
}

void Platform_Settings::setGeneralOption(const QString &sOpt, const QVariant &var)
{
    settings()->setOption(QString("base.general.%1").arg(sOpt), var);
    settings()->sync();
}

void Platform_Settings::crashCheck()
{
    //重置崩溃检测状态位
    settings()->setOption(QString("set.start.crash"),true);
    settings()->sync();
    //崩溃检测
    QTimer::singleShot(1000, this, &Platform_Settings::onSetCrash);
}

QVariant Platform_Settings::generalOption(const QString &sOpt)
{
    return settings()->getOption(QString("base.general.%1").arg(sOpt));
}

QVariant Platform_Settings::internalOption(const QString &sOpt)
{
    return settings()->getOption(QString("base.play.%1").arg(sOpt));
}

void Platform_Settings::setInternalOption(const QString &sOpt, const QVariant &var)
{
    settings()->setOption(QString("base.play.%1").arg(sOpt), var);
    settings()->sync();
}

QString Platform_Settings::forcedInterop()
{
    return internalOption("forced_interop").toString();
}

bool Platform_Settings::disableInterop()
{
