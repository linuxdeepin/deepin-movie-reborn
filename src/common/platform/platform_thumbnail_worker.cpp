...
QPixmap Platform_ThumbnailWorker::genThumbPrecise(const QUrl &url, int secs)
{
    qInfo() << "Generating precise thumbnail for" << url << "at" << secs << "seconds";
...
QPixmap Platform_ThumbnailWorker::genThumb(const QUrl &url, int secs)
{
    ...
    if (!pm.isNull()) {
        qInfo() << "Precise thumbnail generated successfully, size:" << pm.size();
        return pm;
    }
