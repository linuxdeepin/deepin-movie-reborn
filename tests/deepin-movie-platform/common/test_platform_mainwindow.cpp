...
#include "utils.h"
#include "actions.h"
#include "src/common/platform/platform_dbus_adpator.h"
#include "dbusutils.h"
...
TEST(MainWindow, nakedstream)
{
    ...
    Platform_VolumeSlider* volumeSlider = toolboxProxy->volumeSlider();
...
//    Platform_PlaylistWidget *playlistWidget;
...
TEST(MainWindow, DBus)
{
    ...
    DBusUtils utils;
    QVariant v = Platform_ApplicationAdaptor::readDBusProperty("com.deepin.daemon.Audio", "/com/deepin/daemon/Audio",
                                                     "com.deepin.daemon.Audio", "SinkInputs");
    QVariant v_invalid = Platform_ApplicationAdaptor::readDBusProperty("com.test", "/test", "com.test", "SinkInputs");
    v_invalid = DBusUtils::readDBusProperty("com.test", "/test", "com.test", "SinkInputs");

    if (v.isValid()) {
        QList<QDBusObjectPath> allSinkInputsList = v.value<QList<QDBusObjectPath> >();
        for (auto curPath : allSinkInputsList) {
            QVariant name = Platform_ApplicationAdaptor::readDBusProperty("com.deepin.daemon.Audio", curPath.path(),
    ...
    QVariant method = Platform_ApplicationAdaptor::readDBusMethod("com.deepin.daemon.Audio", "/com/deepin/daemon/Audio",
                                                        "com.deepin.daemon.Audio", "SinkInputs");
    QVariant method_invalid = Platform_ApplicationAdaptor::readDBusMethod("com.test", "/com/test", "com.test", "SinkInputs");
    method = DBusUtils::readDBusMethod("com.deepin.daemon.Audio", "/com/deepin/daemon/Audio",
                                      "com.deepin.daemon.Audio", "SinkInputs");
...
TEST(MainWindow, resizeWindow)
{
    ...
    //缩放窗口
    w->move(200, 200);
    w->updateGeometry(Platform_CornerEdge::Platform_LeftEdge, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_TopEdge, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_RightEdge, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_BottomEdge, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_NoneEdge, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_TopLeftCorner, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_TopRightCorner, QPoint(1300, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_BottomLeftCorner, QPoint(100, 100));
    w->updateGeometry(Platform_CornerEdge::Platform_BottomRightCorner, QPoint(100, 100));
}
...
//    w->setTouched(true);
//    Platform_Settings::get().settings()->setOption("base.play.showInthumbnailmode", true);
//#endif
    QTest::qWait(500);
...
//#if !defined (__mips__ ) && !defined(__aarch64__)
//    Platform_Settings::get().settings()->setOption("base.play.showInthumbnailmode", false);
//#endif
    if (w->isFullScreen()) {
        //quit fullscreen
...
TEST(MainWindow, ThumbnailPreview)
{
    ...
    Platform_ThumbnailWorker::get().setPlayerEngine(engine);
    Platform_ThumbnailWorker::get().requestThumb(QUrl("/data/source/deepin-movie-reborn/movie/demo.mp4"),
                                        200);
...
TEST(ToolBox, playListWidget)
{
    ...
    Platform_ToolboxProxy *toolboxProxy = w->toolbox();
    ToolButton *listBtn = toolboxProxy->listBtn();
    Platform_PlaylistWidget *playlistWidget = w->playlist();
...
//    QTest::mouseClick(w, Qt::LeftButton, Qt::NoModifier, QPoint(500, 500), 500);
